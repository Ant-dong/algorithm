...
## B+树 
1. 只有叶子节点存放数据，叶子节点中没有指针，叶子节点中包含所有的key
2. 叶子节点之间加入了顺序访问指针，也就是每个叶子节点增加一个指向相邻叶子节点的指针
3. 非叶子节点中不存储数据，只有key和指向子节点的指针

## B+树相对于B树的优点
1. B+树可以减少IO的访问次数（因为B+树的中间节点不存储数据，使得相同大小的资源可以得到更多的数据，这些数据是在内存中比较，因此减少了IO的访问次数），使得速度更快
2. B+树在查找的时候更加的稳定，因为最终的查找的位置都是在叶子节点
3. B+树在范围查找更加的快速，因为B+树的叶子节点之间存在顺序指针
## 红黑树
...
![youxuan.png](https://github.com/wuzhengdong1/algorithm/blob/master/%E5%B7%A6%E6%97%8B.png)
...
##### 红黑树删除元素总结
![红黑树删除总结](https://github.com/wuzhengdong1/algorithm/blob/master/%E7%BA%A2%E9%BB%91%E6%A0%91%E7%9A%84%E5%88%A0%E9%99%A4%E6%83%85%E6%99%AF.png)
